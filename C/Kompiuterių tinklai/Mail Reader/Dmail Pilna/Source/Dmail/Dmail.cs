...
namespace HigLabo.Mail.SampleApplication
{
    class Dmail
    {
        ...
        private static int ImapMailReceive(string nickname, string password)
        {
            ...
            string data;
            MailMessage mg = null;
            //using (ImapClient cl = new ImapClient("imap.gmail.com", 993, "uabandern@gmail.com", "passwordisreal")) //+
            ...
            {
                cl.Ssl = true; //+
                var bl = cl.Authenticate(); //Grazina true jeiauthenticated
                if (bl == true) //+
                {
                    //Select folder
                    var folder = cl.SelectFolder("INBOX"); //Sudeda informacija apie rezultata i ImapFolder ir ji grazina
