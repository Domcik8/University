...
namespace HigLabo.Net
{
    ...
    public class BufferManager
    {
        ...
        public BufferManager(Int32 poolSize, Int32 bufferSize)  //Sukuriam bufferi duomenyms
        {
            this._PoolSize = poolSize; //+
            this._BufferSize = bufferSize; //+
            this.Initialize(); //Uzpildom Queue poolsize didzio buffersize baitais
        }
        private void Initialize() //Uzpildom Queue poolsize didzio buffersize baitais
        {
            _Buffers = new Queue<Byte[]>(_PoolSize); //+
            for (int i = 0; i < _PoolSize; i++) //+
            {
                _Buffers.Enqueue(new byte[_BufferSize]); //PoolSize uzpildomas Bufferio siza
            }
        }
        /// <summary>
        /// 
        /// </summary>
        /// <returns></returns>
        public Byte[] CheckOut() //Is buferio isimam _DequeueRetryCount baitu
        {
            Int32 count = 1; //+
