...
int main()
{
	...
	eile *ptr;
	eile *el;
	Aktivus *aktivus2 = NULL;				//Aktyviu klientu struktura
	Aktivus *aDelete = NULL;
	int deleter[MAXD] = {0};
	int deletern = -1;
	int kiek = 0, kiekmin = MAX, kuri = 0, l = 0;
	...
	if ((error = Nuskaityti(&laikas, &tikimybe, &darbuotojai, &kaina, &cekis, &pPaimimas, &prPadavimas, &prSurinkimas, &prSurasimas, &kelias, &prGavimas, 
																	&cekismin, &pPaimimasmin, &prPadavimasmin, &prSurinkimasmin, &prSurasimasmin, &keliasmin, &prGavimasmin)) != 0)
	{
		if (error == 1)
	...
	while (dLaikas != laikas)
	{
		...
		if (rand() % 100 <= tikimybe)
		{
			...
			deletern++;
			deleter[deletern] = aktivus;
			aktivus->id = klientoid;
			aktivus->atiejo = dLaikas;
			aktivus->lauke = 0;
			temp1 = cekismin;
			temp1 += rand() % (cekis - cekismin + 1);
			//printf("%d klientui cekio paimimas truks %d\n", aktivus->id, temp1);
			aktivus->pr1 = temp1;
			temp1 = pPaimimasmin;
			temp1 += rand() % (pPaimimas - pPaimimasmin + 1);
		...
		if (aktivus != NULL)
		{
			temp1 = prPadavimasmin;
			temp1 += rand() % (prPadavimas - prPadavimasmin + 1);
			aktivus->pr1 += temp1;
			...
			kiekmin = MAX;
			for (l = 0; l < darbuotojai; l++)
			{
				Get_Count(&kiek, &kasa1[l]);
				if (kiekmin < kiek)
					kuri = kiek;
			}
			temp1 = kuri;
		...
		if (aktivus != NULL)
		{
			...
			temp1 = prGavimasmin;
			temp1 += rand() % (prGavimas - prGavimasmin + 1);
			aktivus->pr23 = temp1;
	...
	for (i = 0; i < darbuotojai; i++)
	{
		Delete_Queue(&kasa1[i]);
		trintiSarasa(&kasa2[i]);
	}
	Delete_Queue(&kasa3);
	return 0;
}
...
int Nuskaityti(int *laikas, int *tikimybe, int *darbuotojai, int *kaina, int *cekis, int *pPaimimas, int *prPadavimas, int *prSurinkimas, int *prSurasimas, int *kelias, int *prGavimas,
	int *cekismin, int *pPaimimasmin, int *prPadavimasmin, int *prSurinkimasmin, int *prSurasimasmin, int *keliasmin, int *prGavimasmin)
{
	...
	*cekis = strtol(line, NULL, 10);
	if (fgets(line, MAX, duomenys) == NULL) return 1;
	*cekismin = strtol(line, NULL, 10);
	if (fgets(line, MAX, duomenys) == NULL) return 1;
	*pPaimimas = strtol(line, NULL, 10);
	if (fgets(line, MAX, duomenys) == NULL) return 1;
	*pPaimimasmin = strtol(line, NULL, 10);
	if (fgets(line, MAX, duomenys) == NULL) return 1;
	*prPadavimas = strtol(line, NULL, 10);
	if (fgets(line, MAX, duomenys) == NULL) return 1;
	*prPadavimasmin = strtol(line, NULL, 10);
	if (fgets(line, MAX, duomenys) == NULL) return 1;
	*prSurinkimas = strtol(line, NULL, 10);
	if (fgets(line, MAX, duomenys) == NULL) return 1;
	*prSurinkimasmin = strtol(line, NULL, 10);
	if (fgets(line, MAX, duomenys) == NULL) return 1;
	*prSurasimas = strtol(line, NULL, 10);
	if (fgets(line, MAX, duomenys) == NULL) return 1;
	*prSurasimasmin = strtol(line, NULL, 10);
	if (fgets(line, MAX, duomenys) == NULL) return 1;
	*kelias = strtol(line, NULL, 10);
	if (fgets(line, MAX, duomenys) == NULL) return 1;
	*keliasmin = strtol(line, NULL, 10);
	if (fgets(line, MAX, duomenys) == NULL) return 1;
	*prGavimas = strtol(line, NULL, 10);
	if (fgets(line, MAX, duomenys) == NULL) return 1;
	*prGavimasmin = strtol(line, NULL, 10);
